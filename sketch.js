...
// preloadは使用しない（エラー時に画面が固まるのを防ぐため）
// ============================================

function setup() {
  logDebug("p5.js setup 開始");
  // モバイル判定
  ...
  logDebug("初期化完了。ユーザーの開始操作を待機中...");
}

// カメラ起動とMLモデル初期化のフロー
async function startCameraFlow() {
  logDebug("startCameraFlow: 開始ボタンがタップされました");
  ...
  try {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      throw new Error("HTTPS接続でない、またはカメラがサポートされていません。");
    }

    logDebug("カメラのアクセス許可をリクエスト中...");
    ...
    handPose = ml5.handPose({ flipped: true }, () => {
      logDebug("ml5.handPose モデルロード完了！");
      modelReady = true;
